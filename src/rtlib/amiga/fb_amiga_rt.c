...
FBSTRING *fb_StrAssign(void *dst, int32 dst_size, void *src, int32 src_size, int32 fill_rem) {
    ...
    if (new_data) {
        if (s_data) memcpy(new_data, s_data, s_len);
        new_data[s_len] = 0;
...
int fb_FileLineInput(int fnum, void *dst, int32 dst_len, int fillrem) {
    ...
    if (d->data) free(d->data);
    d->data = malloc(i + 1);
    if (d->data) { memcpy(d->data, buf, i + 1); d->len = i; d->size = i + 1; }
    else { d->len = 0; d->size = 0; }
    free(buf);
    return 0;
}
...
/* Proper exit - return to AmigaOS */
void exit(int code) {
    /* fb_End should have been called already.
       Just return to the startup code which handles CloseLibrary. */
    __asm volatile ("move.l %0,%%d0; rts" : : "r"(code) : "d0");
