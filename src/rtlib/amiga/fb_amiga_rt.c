...
void fb_PrintInt(int32 fnum, int32 val, int32 mask) {
    ...
    if (val >= 0) {
        if (fnum > 0 && fnum < FB_MAX_FILES && file_handles[fnum])
            dos_Write(file_handles[fnum], " ", 1);
        else dos_Write_buf(" ", 1);
    }
    ...
    if (fnum > 0 && fnum < FB_MAX_FILES && file_handles[fnum]) {
        dos_Write(file_handles[fnum], out, i);
        if (mask & 1) dos_Write(file_handles[fnum], "\n", 1);
    } else {
        dos_Write_buf(out, i);
        if (mask & 1) dos_Write_buf("\n", 1);
    }
...
/* exit() - in normal flow, fb_End returns to main which returns to startup.s.
   If called directly (e.g. from runtime error), we cannot easily unwind to
   startup.s cleanup. Use dos.library Exit() which terminates the process. */
extern void *DOSBase;
void exit(int code) {
    register void *a6 __asm("a6") = DOSBase;
    register long d1 __asm("d1") = code;
    __asm volatile ("jsr -144(%%a6)" : : "r"(a6), "r"(d1) : "d0","a0","a1","memory");
